// @polsia:user-owned — POST /api/forge/missions/:id/rollback.

import 'server-only';
import { NextResponse } from 'next/server';
import type { z } from 'zod';
import { forgeErrorResponse, requireForgeAuth } from '@/lib/business/forge/api-helpers';
import { rollbackMission } from '@/lib/business/forge/service';
import { RollbackRequest } from '@/lib/contracts/forge';
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const detail = await rollbackMission({
      missionId: id,
      userId: auth.user.id,
      isAdmin: auth.isAdmin,
      toStageHandoffId: parsed.data.toStageHandoffId,
