// @polsia:user-owned — POST /api/forge/missions/:id/resume.

import 'server-only';
import { NextResponse } from 'next/server';
import type { z } from 'zod';
import { forgeErrorResponse, requireForgeAuth } from '@/lib/business/forge/api-helpers';
import { resumeMission } from '@/lib/business/forge/service';
import { ResumeRequest } from '@/lib/contracts/forge';
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const parsed = ResumeRequest.safeParse(await req.json().catch(() => ({})));
  if (!parsed.success) {
    return NextResponse.json(fieldErrorBody(parsed.error), { status: 400 });
  }
  const { id } = await ctx.params;
  try {
    const detail = await resumeMission({
      missionId: id,
      userId: auth.user.id,
