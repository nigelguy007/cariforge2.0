...
import { replayMission } from '@/lib/business/forge/service';
import { ReplayRequest } from '@/lib/contracts/forge';
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const parsed = ReplayRequest.safeParse(await req.json().catch(() => ({})));
  if (!parsed.success) {
    return NextResponse.json(fieldErrorBody(parsed.error), { status: 400 });
  }
  const { id } = await ctx.params;
  try {
    const detail = await replayMission({
      missionId: id,
      userId: auth.user.id,
      isAdmin: auth.isAdmin,
      fromStageIndex: parsed.data.fromStageIndex,
