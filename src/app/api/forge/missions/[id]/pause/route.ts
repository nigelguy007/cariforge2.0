...
import { pauseMission } from '@/lib/business/forge/service';
import { PauseRequest } from '@/lib/contracts/forge';
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const parsed = PauseRequest.safeParse(await req.json().catch(() => ({})));
  if (!parsed.success) {
    return NextResponse.json(fieldErrorBody(parsed.error), { status: 400 });
  }
  const { id } = await ctx.params;
  try {
    const detail = await pauseMission({
      missionId: id,
      userId: auth.user.id,
